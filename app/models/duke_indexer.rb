...
class DukeIndexer < FranklinIndexer
  ...
  def link_to_source_context(rec)
    ...
    "https://find.library.duke.edu/trln/#{system_id}"
  end
  ...
  def get_oclc_id_values(rec)
    rec.fields('035')
      ...
      field.find_all { |sf| subfield_a_is_oclc(sf) }.map do |sf|
        m = /^\s*(\(OCoLC\)[^1-9]*|0*)([0-9]+).*$/.match(sf.value)
        if m
          m[2]
        end
      end.compact
