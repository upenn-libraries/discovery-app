...
class ColumbiaIndexer < FranklinIndexer
  ...
  def get_namespaced_id(rec)
    id = get_001_id(rec)
    id.blank? ? nil : "COLUMBIA_#{id}"
  end

  def link_to_source_context(rec)
    system_id = get_001_id(rec)
    "https://clio.columbia.edu/catalog/#{system_id}"
  end
  ...
  def define_full_text_link_a
    to_field 'full_text_link_a' do |rec, acc|
      ...
      links << {
        linktext: 'View record in Columbia\'s catalog',
