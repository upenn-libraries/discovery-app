...
class PrincetonIndexer < FranklinIndexer

  def define_record_source_id
    to_field 'record_source_id' do |rec, acc|
      acc << RecordSource::PRINCETON
    end
  end
  ...
  def get_namespaced_id(rec)
    id = get_001_id(rec)
    id.blank? ? nil : "PRINCETON_#{id}"
  end

  def link_to_source_context(rec)
    system_id = get_001_id(rec)
    "https://catalog.princeton.edu/catalog/#{system_id}"
  end
  ...
  def define_full_text_link_a
    to_field 'full_text_link_a' do |rec, acc|
      ...
      links << {
        linktext: 'View record in Princeton\'s catalog',
