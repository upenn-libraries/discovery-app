#!/bin/bash
#
# Usage: ./index_preprocess_oai.rb "/path/to/files/*.xml"
#
...
time ./process_files.rb -x $XSL_DIR -p 4 -i "$1" convert_oai_to_marc create_bound_withs
...
time ./process_files.rb -a -x $XSL_DIR -p 4 -i "$GLOB_DIR/*_marc.xml" fix_marc merge_bound_withs format rename_to_final_filename
